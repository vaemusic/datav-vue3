<template>
  <div class="dv-decoration-1" ref="dvDecoration1">
    ...
    <svg :width="`${svgWH[0]}px`" :height="`${svgWH[1]}px`"
      ...
      <template v-for="point in state.points" :key="point">
        <rect v-if="Math.random() > 0.6" :fill="state.mergedColor[0]" :x="point[0] - halfPointSideLength"
          :y="point[1] - halfPointSideLength" :width="pointSideLength" :height="pointSideLength">
          <animate v-if="Math.random() > 0.6" attributeName="fill" :values="`${state.mergedColor[0]};transparent`"
            dur="1s" :begin="Math.random() * 2" repeatCount="indefinite" />
        </rect>
      </template>

      <rect v-if="state.rects[0]" :fill="state.mergedColor[1]" :x="state.rects[0][0] - pointSideLength"
        ...
        <animate attributeName="x" :values="`${state.rects[0][0]};${state.rects[0][0] - pointSideLength}`" dur="2s"
          repeatCount="indefinite" />
        <animate attributeName="y" :values="`${state.rects[0][1]};${state.rects[0][1] - pointSideLength}`" dur="2s"
          repeatCount="indefinite" />
      </rect>

      <rect v-if="state.rects[1]" :fill="state.mergedColor[1]" :x="state.rects[1][0] - 40"
        ...
        <animate attributeName="x" :values="`${state.rects[1][0]};${state.rects[1][0] - 40};${state.rects[1][0]}`"
...
const calcPointsPosition = () => {
  ...
  let newPoints: Array<any> = new Array(rowNum.value).fill(0).map((foo, i) =>
